...
class BookingService {
  ...
  async sendMailToCaregiver(client, caregiverIds, booking) {
    let caregiver = await Caregiver.query()
      .select('id', 'user_id', 'registration_no', 'english_name', 'chinese_name', 'nick_name')
    ...
    if (caregiver) {
      ...
      for (const c of caregiver) {
        ...
        mailData.acceptLink = `${Env.get('BOOKING_ACCEPT_BY_CG_LINK')}?booking_id=${encryptedData}`;
        mailData.rejectLink = `${Env.get('BOOKING_REJECT_BY_CG_LINK')}?booking_id=${encryptedData}`;
  ...
  async cronForFeedbackForm() {
    try {
      ...
      for (const data of bookingData) {
        ...
        encryptedData = CryptoJS.enc.Base64.parse(encryptedData);
        encryptedData = encryptedData.toString(CryptoJS.enc.Hex);
        mailData.link = `${Env.get('FEEDBACK_FORM_CLIENT')}?booking=${encryptedData}`;
        // mail for client
        await Mail.send(
