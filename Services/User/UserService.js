...
class UserService {
  ...
  async deleteUser(user) {
    try {
      ...
      //   .whereIn('id', user)
      //   .update({ is_deleted: 1, deleted_at: new Date() });
      const deleteMark = `-deleted-${Date.now()}`;
      const deleteuser = await Database
        .raw(`update users set email = CONCAT(email, '${deleteMark}'), is_deleted = 1, deleted_at = now(), updated_at = now() where id in (${user.join(',')})`)
      return deleteuser;
    } catch (error) {
