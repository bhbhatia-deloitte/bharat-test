  ...
  async addFeedback(reqData, checkAccepted) {
    ...
    try {
      ...
      if (!reqData.negative_feedback && Number(setting.toJSON().value) > 0) {
        ...
        if (caregiver.refferers_email && caregiver.total_completion_event < 4) {
          const findNegative = caregiver.feedbacks.filter(
            e => e.negative_feedback != null,
          );
          ...
          if (user) {
            if (
              !findNegative.length ||
              (findNegative.length && findNegative.length < 3)
            ) {
              if (caregiver.referrals.length <= 2) {
